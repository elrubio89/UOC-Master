...
VN: no hay (todas las secuencias son genes de resistencia)
...
```
### RGI:
9473 secuencias mapeadas: (mapping stats)
  232 mapeadas por 2-1 pipeline (len de RGI_seqs_bam)
    224 mapeadas correctamente
    8 mapeadas incorrectamente
  9241 mapeadas por los 3 pipelines (9473-232)
527 secuencias no mapeadas
...
### GROOT:
```{bash}
samtools view groot_analysis/argannot-150bp-10000-reads-uniqseq.bam | wc -l
```
```{r message=FALSE, warning=FALSE}
...
8822 secuencias mapeadas: (bam file)
  694 mapeadas por 2-1 pipeline (len de Groot_seqs_bam)
    680 mapeadas correctamente
    14 mapeadas incorrectamente
  8128 mapeadas por los 3 pipelines (8822-694)
1178 secuencias no mapeadas
