...
```{r}
report <- read_delim("~/Documentos/UOCMaster/RTC/ariba_analysis/ariba_results/report.tsv", 
...
```{r}
library(readr)
aro_index <- read_delim("~/Documentos/UOCMaster/CARD_data/aro_index.tsv", 
    "\t", escape_double = FALSE, trim_ws = TRUE)
```
...
```{r}
AMR_family_ARIBA$`AMR Gene Family`
AMR_family_RGI$`AMR Gene Family`
AMR_family<-merge(AMR_family_ARIBA, AMR_family_RGI, by="AMR Gene Family", all=TRUE)
