...
comparison_df$pipelines<-as.factor(comparison_df$pipelines)
```
...
```{r}
ggplot(comparison_df, aes(pipelines, sum_RPKM_RGI))+geom_boxplot(aes(colour=npipelines), outlier.shape = NA, na.rm = TRUE)+geom_jitter(width = 0.2, aes(colour=npipelines))+theme_classic()+theme(legend.position = "none")+ggtitle("Rgi")
```
